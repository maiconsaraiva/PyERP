...
class PyInvoice(PyFather):
    ...
    type = models.IntegerField(_("Type"), choices=INVOICE_TYPE, null=True, blank=True)
    partner_id = models.ForeignKey(
        PyPartner, on_delete=models.PROTECT, verbose_name=_("Client")
    )
    ...
    sale_order_id = models.ForeignKey(
        PySaleOrder, null=True, blank=True, on_delete=models.PROTECT
    )
    purchase_order_id = models.ForeignKey(
        PyPurchaseOrder, null=True, blank=True, on_delete=models.PROTECT
    )
    date_invoice = models.DateTimeField(default=timezone.now)
    amount_untaxed = models.DecimalField(
        _("Amount un"), max_digits=100, decimal_places=2, default=0
    )
    amount_tax_iva = models.DecimalField(max_digits=100, decimal_places=2, default=0)
    ...
    state = models.ForeignKey(
        PyInvoiceState, on_delete=models.PROTECT, verbose_name=_("State"), default=1
    )
...
class PyInvoiceDetail(PyFather):
    ...
    invoice_id = models.ForeignKey(PyInvoice, on_delete=models.PROTECT)
    product_id = models.ForeignKey(
        PyProduct, on_delete=models.PROTECT, verbose_name=_("Product")
    )
    description = models.TextField(blank=True, null=True)
    quantity = models.DecimalField(
        _("Quantity"), max_digits=100, decimal_places=2, default=0
    )
    uom_id = models.ForeignKey(
        PyUom, verbose_name=_("UOM"), null=True, blank=True, on_delete=models.PROTECT
    )
    price = models.DecimalField(_("Price"), max_digits=100, decimal_places=2, default=0)
